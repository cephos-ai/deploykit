...
workaround, isolate hacks to `deploy/<customer>/<slug>` branches, and log the
context in the Debt Log - in the same commit, with customer data scrubbed.
...
3. appends a sanitized Debt Log entry to `DEPLOY.md` in the same commit,
4. leaves the retro verdict `_pending_` for the [retro](../../protocols/RETRO.md) to resolve.
